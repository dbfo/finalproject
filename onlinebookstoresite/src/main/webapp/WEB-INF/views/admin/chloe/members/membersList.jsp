...
<div class="container-fluid">
	...
	<div style="font-size: 2rem; margin-left: 25px">
		<i class="fas fa-table mr-1"></i> 회원검색
	</div>
	<br>

	<div class="container-fluid">
		<form action="${pageContext.request.contextPath }/membersList" method="post">
			<table class="table">
				<tr>
	...
	<div style="font-size: 2rem; margin-left: 25px">
		...
		<span style="margin-left: 1150px;"> <button onclick="location.href='${pageContext.request.contextPath}/membersExcelDownload'" type="button" class="btn btn-outline-success">엑셀 다운로드</button></span>
	</div>
	<br>
		...
		<div id="listPaging">
			<c:choose>
				<c:when test="${pu.startPageNum > 1 }">
					<button onclick="location.href='${pageContext.request.contextPath }/membersList?pageNum=${pu.startPageNum - 1}'" 
				...
				<c:choose>
					<c:when test="${i == pu.pageNum }">
						<button type="button" class="btn btn-outline-success" 
							onclick="location.href='${pageContext.request.contextPath }/membersList?pageNum=${i}&infoField=${infoField}&infoKeyword=${infoKeyword}&ageKeyword1=${ageKeyword1}&ageKeyword2=${ageKeyword2}&dateKeyword1=${dateKeyword1}&dateKeyword2=${dateKeyword2}'">${i }</button>
					</c:when>
					<c:otherwise>
						<button type="button" class="btn btn-outline-success" 
							onclick="location.href='${pageContext.request.contextPath }/membersList?pageNum=${i}&infoField=${infoField}&infoKeyword=${infoKeyword}&ageKeyword1=${ageKeyword1}&ageKeyword2=${ageKeyword2}&dateKeyword1=${dateKeyword1}&dateKeyword2=${dateKeyword2}'">${i }</button>
			...
			<c:choose>
				<c:when test="${pu.totalPageCnt > pu.endPageNum }">
					<button onclick="location.href='${pageContext.request.contextPath }/membersList?pageNum=${pu.endPageNum + 1}'" 
